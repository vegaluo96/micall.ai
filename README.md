...
- **流式管线（首句抢跑，~1.2–1.6s 首音）**：实时 ASR（Qwen3-ASR）→ 快脑 LLM（DeepSeek-V4-Flash，禁用思考、首句一成形即合成）→ TTS（MiniMax，逐句合成 + 并行预合成消除句间空档）。每句情绪 `[emotion:tag]` piggyback 在 LLM 输出里、不额外调模型；笑/叹气用拟声标签真发声。回声/语气词/ASR 幻听过滤防自打断与凭空冒话；`frequency_penalty` + 反复读护栏压"车轱辘话"。
...
| 情节记忆 | 语义召回：相似度 × **重要性** × 情感权重（时间新近为辅 —— 优先想起 TA 真在意的事） | Postgres + `pgvector`，Bailian embedding |
...
离线"慢脑"（DeepSeek-V4-Pro 推理模型）在挂断后回写画像、推进自主状态与世界，并发封顶不抢在线接话 —— **"她有对话之外的生命"**。

## 模型阵容（节点可在后台「接口配置」热改，留空 key 自动回退）

| 节点 | 模型 | 用途 |
|---|---|---|
| `asr` | Qwen3-ASR-Flash（百炼） | 实时语音识别 |
| `llm_fast` | DeepSeek-V4-Flash | 通话快脑（禁思考、流式、首句抢跑） |
| `tts` | MiniMax speech-2.8-turbo | 逐句合成、情绪韵律、拟声 |
| `llm_slow` | DeepSeek-V4-Pro | 离线理解 / 自主状态推进（慢脑） |
| `embedding` | text-embedding-v4（百炼） | 情节记忆语义召回 |
| `llm_eval` | GPT-4o（经 apiyi） | 图灵测试裁判 / 后台「AI 生成角色·内核」 |
| `llm_search` | Qwen-Long（经 apiyi） | 真实热点改写成口语（grounded，不联网编造） |
