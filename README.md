...
- **角色塑造的第一性原理**：角色不是一张平铺属性表，而是**以内核（spine）为根**——先定"这个人最在乎/最怕失去的那件事 + 守着的软处"，性格 / 说话 / 好恶都从内核长出来、能回溯到它。这条贯穿所有创作入口：出厂 38 角色、后台 **AI 两段式生成（先立魂 → 再从魂长出全身）**、给外部 AI 的「复制模板」，遵同一把尺；判据是"电话里听得见/感觉得到的才写实"，卷尺数据与 MBTI 可留白、不硬编。
- **会学习的陪伴**：挂断后离线"慢脑"回写事实/画像/洞察、推进角色自己的生活；用户的**通话评价**也会回写到画像，下一通让 AI 据真人反馈校准对你的表现。
- **运营后台**：角色管理（人设 / 内核 / 说话风格 / 基础资料 / 音色全字段可编辑、可新建、可 **AI 两段式生成**、可音色克隆、**可一键同步出厂口吻**——把被后台覆盖过的实时口吻拉回仓库最新出厂值、其它覆盖保留）、接口配置（endpoint/key 存服务端）、用户 / 订单 / 通话（含转写内容）/ 成本看板、世界库与热点源体检、邀请裂变、工单、手动增减时长、分页。
...
- **流式管线（首句抢跑，~1.2–1.6s 首音）**：实时 ASR → 快脑 LLM（流式、禁用思考、首句一成形即合成）→ TTS（逐句合成 + 并行预合成消除句间空档）。每句情绪 `[emotion:tag]` piggyback 在 LLM 输出里、不额外调模型；笑/叹气用拟声标签真发声。回声/语气词/ASR 幻听过滤防自打断与凭空冒话；`frequency_penalty` + 反复读护栏（**引用上一轮原话、明令不重复**）+ **每角色专属反口癖护栏** 压"车轱辘话"。
...
```bash
cd backend
PYTHONPATH=src python3 -m unittest discover -s tests -q   # 单元测试（397 passed）
