...
_CN_ACTION = re.compile(r"（[^）]*）|【[^】]*】|\*[^*]*\*")        # 中文旁白/动作/星号（配对）：一律去掉
# 跨句/未闭合的旁白残留：模型把「（轻笑…敲了两下。）正经话」写成带句号的多句旁白，逐句切后会留下
# 「（…（无闭括号）」「）…（无开括号）」这种半拉括号——配对正则吃不掉。这里兜底：未闭合的开符删到结尾、
# 残留的孤立全角闭符直接清掉。保证任何半拉旁白都不漏进字幕/被 TTS 念出来。
_UNCLOSED_ACTION = re.compile(r"（[^）]*$|【[^】]*$|\([^)]*$|\*[^*]*$")
_ORPHAN_CLOSE = re.compile(r"[）】]")


def _strip_actions(t: str) -> str:
    """去配对旁白 + 未闭合旁白 + 孤立闭符。中文全角括号在语音陪伴里几乎只会是舞台说明，安全清除。"""
    t = _CN_ACTION.sub("", t)
    t = _UNCLOSED_ACTION.sub("", t)
    t = _ORPHAN_CLOSE.sub("", t)
    return t
_ALL_EMOTION_TAGS = re.compile(                                  # 句中任意位置的方括号标注残留（情绪标签 / 舞台说明）
    # 首字符仍要求字母/汉字（数字开头如 [8:30] 不当标签，不误伤时间）；但首字符【之后】允许任意非闭合括号字符——
    # 含逗号/句号/全角标点（用户实测：「[sighs，有点无奈地笑了]」因全角逗号 ，不在旧字符类里，整条漏进字幕）。
    r"[\[【]\s*(?:[a-zA-Z_一-鿿]{1,12}\s*[:：]\s*)?[a-zA-Z_一-鿿][^\]】]{0,60}[\]】]"
)
# 模型有时把笑/叹写成【方括号多词】形式（[laughs softly] / [sighs，有点无奈地笑了]）：既漏进字幕、又不被
# MiniMax 当拟声念。送 TTS 前把这类（首词是 sigh/laugh 系）转成 MiniMax 认的拟声 (laughs)/(sighs)，让她真笑真叹；
# 字幕侧由上面的标签清洗一并去掉。内部允许任意非闭合字符（含逗号/中文），不被标点截断。
_BRACKET_LAUGH = re.compile(r"[\[【]\s*(?:laugh|chuckl|giggl|haha|hehe|grin)[^\]】]*[\]】]", re.IGNORECASE)
_BRACKET_SIGH = re.compile(r"[\[【]\s*(?:sigh|exhale)[^\]】]*[\]】]", re.IGNORECASE)
...
def clean_for_tts(text: str) -> str:
    ...
    t = _strip_actions(t)                            # 中文旁白（含跨句/未闭合/孤立闭符）一律去掉
...
def clean_for_subtitle(text: str) -> str:
    ...
    t = _PAUSE.sub("", t)
    t = _strip_actions(t)       # 中文旁白（含跨句切碎的「（轻笑…敲了两下。」「）正经话」半拉括号）一律去掉
