...
CREATE TABLE IF NOT EXISTS calls (
    ...
    guest_ip         TEXT                             -- 游客（未注册）通话的来访 IP（归属地）；登录用户为空，用 user_id 即可
);
...
-- 兼容已有库：补 guest_ip 列（幂等）。游客通话记录归属地 IP；登录用户为空。
ALTER TABLE calls ADD COLUMN IF NOT EXISTS guest_ip TEXT;
