...
def _shared_client() -> "httpx.AsyncClient":
    """共享 HTTP 连接池，但【按事件循环隔离】（与 minimax_tts 同一修复）。
    关键：管理端连通性测试走 asyncio.run（每次一次性事件循环），通话走 wsserver 常驻循环。同一个 httpx
    client 跨事件循环复用会污染连接池 → 连接挂死不释放 → 攒满后 PoolTimeout（本会话已在 TTS 侧实测踩过）。
    故按当前运行循环隔离：循环变了就建新 client（旧的随其循环关闭而释放），并加限额/回收/短 pool 超时。"""
