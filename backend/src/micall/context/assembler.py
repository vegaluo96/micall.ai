...
_ADDRESSEE = (
    ...
    "TA 一旦说了、或纠正你，就立刻记住、之后绝不再叫错、再安错身份。"
)
...
def _autonomous_block(s: AutonomousState) -> str:
    ...
    return (
        ...
        + "\n".join(bits)
        + "\n（这些是【你自己】的事——电话那头的 TA 多半并不知情，更不是其中的一员。你可以像跟朋友唠近况、发发牢骚那样"
          "主动说给 TA 听，但【绝不能】把 TA 当成你这摊事里的人：别问 TA「那个谁档期定了没」「项目进度怎么样了」这类"
          "只有当事人才答得上的话，也别预设 TA 手里有跟你平行的同类活儿——除非 TA 自己说过 TA 也在这行、或正经历着同样的事。）"
    )
